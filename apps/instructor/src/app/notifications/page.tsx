...
import { Button, Card, Badge, Skeleton, toast } from '@3de/ui';
...
export default function NotificationsPage() {
  ...
  const { data: notificationsResponse, isLoading , refetch } = useQuery({
    queryKey: ['notifications', skip, take, search],
    queryFn: async () => {
      const response = await notificationApi.getAllByUserId(user?.id as string,skip,take,search);
      return response;
    },
  });
  ...
  const handleDeleteAll = async () => {
    try {
      toast.success('جاري حذف التنبيهات');
      notifications.forEach(async (notification: Notification) => {
        await notificationApi.delete(notification.id);
      });
      toast.success('تم حذف التنبيهات بنجاح');
    } catch (error) {
      toast.error('حدث خطأ أثناء حذف التنبيهات');
      console.error('Error deleting notifications:', error);
    }
  };
  const handleMarkAllAsRead = async () => {
    try {
      toast.success('جاري تحديث التنبيهات');
      notifications.forEach(async (notification: Notification) => {
      ...
      <div className="space-y-6">
        ...
        {!isLoading && (!filteredNotifications || filteredNotifications.length === 0) && (
          ...
          >
           ...
           {/* Pagination */}
           {notifications && notifications.length && notifications.length > 0 && notificationsResponse?.data?.totalPages && notificationsResponse?.data?.totalPages > 0 &&  (
          <Pagination
