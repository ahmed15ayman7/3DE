...
import { z } from 'zod'
import { Card, Button, Input, Textarea, Select, Badge, Switch, RouteLoader } from '@3de/ui'
 ...
 function NewQuizPage() {
  ...
  useEffect(() => {
    if(searchParams.get('courseId')){
      setValue('courseId', searchParams.get('courseId')||'')
    }
  }, [])
...
export default function NewQuizPageSearchParams() {
  return <Suspense fallback={<RouteLoader loadingText='جاري تحميل الصفحة' />}>
    <NewQuizPage />
  </Suspense>
}